...
import {
  ...
  Home, ClipboardList, ScanLine, ShieldCheck, UserCheck, UserPlus2, GraduationCap
} from 'lucide-react';
...
export default function KelolaAkun() {
      ...
      if (editingId) {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            password,
            tipe,
            tanggal_expired: (tipe === 'tamu' || tipe === 'siswa') ? (tanggalExpired || null) : null
          })
          .eq('id', editingId);
        if (error) throw error;
      } else {
  ...
  const getTipeBadge = (t) => {
    if (t === 'admin') return { text: 'Lead Instructor', className: 'bg-rose-500/20 text-rose-400 border-rose-500/40' };
    if (t === 'tamu') return { text: 'Tamu', className: 'bg-amber-500/20 text-amber-400 border-amber-500/40' };
    if (t === 'siswa') return { text: 'Student', className: 'bg-indigo-500/20 text-indigo-400 border-indigo-500/40' };
    return { text: 'Member', className: 'bg-cyan-500/20 text-cyan-400 border-cyan-500/40' };
  };
            ...
            <form onSubmit={handleSimpan} className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { key: 'admin', label: 'Lead Instructor', icon: ShieldCheck },
