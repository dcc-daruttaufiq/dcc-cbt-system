...
export default function KelolaAkun() {
  ...
  const handleSimpan = async (e) => {
    ...
    if (!namaDepan.trim() || !namaBelakang.trim())
      return alert("Nama depan & belakang wajib diisi!");
    if (!password.trim()) return alert("Password wajib diisi!");
    if (tipe === "tamu" && !tanggalExpired)
      return alert("Tanggal expired wajib diisi untuk akun Tamu!");
    ...
    try {
      if (editingId) {
        ...
        const usernameFinal = await generateUsernameUnik(
          namaDepan,
          namaBelakang,
        );
        const { error } = await supabase.from(AKUN_TABLE).insert({
          ...
          status: "aktif",
          tanggal_expired:
            tipe === "tamu" || tipe === "siswa" ? tanggalExpired || null : null,
        });
        if (error) throw error;
        alert(
  ...
  const toggleStatusAkun = async (akun) => {
    ...
    try {
      ...
      if (error) throw error;
      setDaftarAkun((prev) =>
        prev.map((a) => (a.id === akun.id ? { ...a, status: statusBaru } : a)),
      );
    } catch (err) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Admin, Anggota, dan Tamu — login web DCC
                </p>
              </div>
            </div>
            <Button
              onClick={openTambahModal}
              className="text-xs bg-primary hover:opacity-90 text-background font-display font-bold border-0 flex items-center gap-1.5"
            >
          ...
          <div className="max-w-5xl mx-auto space-y-5">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex gap-1.5 bg-surface p-1.5 rounded-xl border border-borderCustom text-xs font-display font-bold">
                {[
                  ...
                  { key: "admin", label: "Lead Instructor" },
                  { key: "anggota", label: "Member" },
                  { key: "siswa", label: "Student" },
                  { key: "tamu", label: "Tamu" },
                ].map((f) => (
                  <button
                    key={f.key}
            ...
            <div className="space-y-3">
              {isLoading ? (
                ...
                <div className="p-10 text-center text-xs text-slate-500 bg-surface/40 rounded-2xl border border-borderCustom">
                ...
                daftarTerfilter.map((akun) => {
                  ...
                  return (
                    <div
                      key={akun.id}
                      className="p-4 bg-surface/60 border border-borderCustom rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                    >
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge
                          className={`text-[9px] px-2 py-0.5 rounded-md font-bold border ${badge.className}`}
                        >
                          {badge.text}
                        </Badge>
                        ...
                        {akun.tipe === "tamu" && akun.tanggal_expired && (
                          <span
                            className={`text-[10px] font-mono px-2 py-0.5 rounded-md border ${expired ? "text-rose-400 border-rose-500/40 bg-rose-500/10" : "text-slate-400 border-slate-700 bg-slate-800/40"}`}
                          >
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-slate-400 hover:text-primary hover:bg-slate-800 transition"
                          title="Edit"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-surface border border-borderCustom rounded-2xl max-w-md w-full p-6 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-borderCustom/60">
              <h3 className="font-display text-base font-bold text-primary uppercase">
            ...
            <form onSubmit={handleSimpan} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-display font-bold text-slate-300 mb-1.5 block uppercase">
                  Username {editingId ? "(tidak berubah)" : "(preview)"}
                </label>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    { key: "anggota", label: "Member", icon: UserCheck },
                    { key: "siswa", label: "Student", icon: GraduationCap },
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-1 py-2.5 rounded-xl border text-xs font-display font-bold transition ${
                        tipe === t.key
                          ...
                          : "text-slate-400 border-borderCustom hover:text-white"
                      }`}
                    >
              ...
              {(tipe === "tamu" || tipe === "siswa") && (
                <div>
                  <label className="text-xs font-display font-bold text-amber-400 mb-1.5 block uppercase">
                    Tanggal Expired {tipe === "siswa" ? "(opsional)" : ""}
                  </label>
                  <input
                    type="date"
              ...
              <div>
                <label className="text-xs font-display font-bold text-slate-300 mb-1.5 flex items-center justify-between uppercase">
                  ...
                  <button
                    type="button"
                    onClick={() => setPassword(generatePasswordAcak())}
                    className="text-primary text-[10px] flex items-center gap-1 normal-case font-sans hover:underline"
                  >
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-borderCustom/60">
                ...
                <Button
                  ...
                  className="bg-primary text-background font-display font-bold text-xs border-0 flex items-center gap-1.5"
                >
