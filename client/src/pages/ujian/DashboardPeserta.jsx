...
const DEFAULT_KATALOG = [
  {
    id: "word",
    nama: "Microsoft Word",
    subNama: "Pengolahan Dokumen & Surat",
    kategori: "Spesialisasi",
    durasi: "90 Menit",
    tokenDefault: "WORD2026",
    bobot_pg: 50,
    bobot_praktik: 50,
    kkm: 75,
  },
  {
    ...
    subNama: "Pengolahan Data & Formula",
    kategori: "Spesialisasi",
    durasi: "90 Menit",
    tokenDefault: "EXCEL2026",
    bobot_pg: 50,
    bobot_praktik: 50,
    kkm: 75,
  },
  {
    ...
    subNama: "Desain Presentasi Interaktif",
    kategori: "Spesialisasi",
    durasi: "90 Menit",
    tokenDefault: "PPT2026",
    bobot_pg: 50,
    bobot_praktik: 50,
    kkm: 75,
  },
  {
    id: "desain",
    nama: "Desain Grafis",
    subNama: "Canva & Visual Typography",
    kategori: "Spesialisasi",
    durasi: "90 Menit",
    tokenDefault: "DESAIN2026",
    bobot_pg: 40,
    bobot_praktik: 60,
    kkm: 75,
  },
  {
    ...
    subNama: "HTML, CSS, & Logic JavaScript",
    kategori: "Spesialisasi",
    durasi: "120 Menit",
    tokenDefault: "CODING2026",
    bobot_pg: 40,
    bobot_praktik: 60,
    kkm: 75,
  },
...
export default function DashboardPeserta() {
        ...
        if (data && data.value) {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
            catalogArr = parsed.map((item) => ({
              ...
              bobot_pg:
                item.bobot_pg !== undefined ? Number(item.bobot_pg) : 50,
              bobot_praktik:
                item.bobot_praktik !== undefined
                  ? Number(item.bobot_praktik)
                  : 50,
              kkm:
                item.kkm !== undefined && item.kkm !== null
                  ? Number(item.kkm)
                  : 75,
            }));
          }
    ...
    const initDashboard = async () => {
      ...
      if (activeUser.tech_id) {
        try {
          ...
          const localSesi = JSON.parse(
            localStorage.getItem(STORAGE_KEYS.PESERTA) || "[]",
          );
          const matchedFromCache = localSesi.find(
            (p) =>
              p.tech_id?.toLowerCase().trim() ===
              activeUser.tech_id?.toLowerCase().trim(),
          );
          if (matchedFromCache) {
      ...
      let tokenIndividuToDisplay =
        activeUser?.token || activeUser?.token_peserta || "";
      if (!tokenIndividuToDisplay && techIdVal) {
        try {
          const savedTokenMap = JSON.parse(
            localStorage.getItem("dcc_persistent_tokens") || "{}",
          );
          const foundKey = Object.keys(savedTokenMap).find(
            (k) => k.toLowerCase().trim() === techIdVal.toLowerCase().trim(),
          );
          if (foundKey) {
      ...
      const rawKat =
        activeUser?.kategori ||
        localStorage.getItem(STORAGE_KEYS.USER_KATEGORI) ||
        "";
      ...
      if (!initialKat) {
        setDataError(
          `Kategori ujian untuk akun ini tidak valid ("${rawKat || "-"}"). Silakan hubungi Pengawas untuk memperbaiki data.`,
        );
        return;
      }
      ...
      const isFinished =
        activeUser?.status === "selesai" ||
        activeUser?.status_koreksi === "SELESAI" ||
        localStorage.getItem(STORAGE_KEYS.IS_EXAM_FINISHED) === "true";
      ...
      if (isFinished) {
        ...
        try {
          ...
          if (dataSoal && Array.isArray(dataSoal)) {
            const filteredSoal = dataSoal.filter(
              (s) => normalizeKategori(s.kategori) === initialKat,
            );
        ...
        const nilaiPG =
          activeUser?.nilai_pg !== undefined && activeUser?.nilai_pg !== null
            ? Number(activeUser.nilai_pg)
            : 0;
        const nilaiPraktik =
          activeUser?.nilai_praktik !== undefined &&
          activeUser?.nilai_praktik !== null
            ? Number(activeUser.nilai_praktik)
            : null;
        const jumlahBenar =
          activeUser?.jumlah_benar !== undefined &&
          activeUser?.jumlah_benar !== null
            ? Number(activeUser.jumlah_benar)
            : null;
        const jumlahSalah =
          activeUser?.jumlah_salah !== undefined &&
          activeUser?.jumlah_salah !== null
            ? Number(activeUser.jumlah_salah)
            : null;
        const jumlahPindahTab =
          activeUser?.jumlah_pindah_tab !== undefined &&
          activeUser?.jumlah_pindah_tab !== null
            ? Number(activeUser.jumlah_pindah_tab)
            : 0;

        let totalNilai =
          activeUser?.nilai_akhir !== undefined &&
          activeUser?.nilai_akhir !== null
            ? Number(activeUser.nilai_akhir)
            : null;

        if (totalNilai === null && nilaiPraktik !== null) {
          const bPG = activeExam?.bobot_pg ?? 50;
          const bPrak = activeExam?.bobot_praktik ?? 50;
          totalNilai = Math.round(
            nilaiPG * (bPG / 100) + nilaiPraktik * (bPrak / 100),
          );
        }
        ...
        setCompletedExamInfo({
          namaUjian: activeExam
            ? activeExam.nama
            : getLabelKategori(initialKat),
          nilaiPG: nilaiPG,
  ...
  const handleMulaiUjian = async (e) => {
    ...
    if (!tokenInput.trim())
      return setTokenError("Masukkan Token Ujian terlebih dahulu!");
    if (!isAgreed)
      return setTokenError(
        "Anda harus menyetujui tata tertib pengerjaan ujian!",
      );
    ...
    try {
      ...
      if (statusSesi !== "DIBUKA") {
        setIsLoading(false);
        return setTokenError(
          "AKSES DITOLAK: Sesi ujian saat ini sedang DITUTUP oleh pengawas!",
        );
      }
    } catch (err) {
      console.warn(
        "Gagal mengecek status sesi ujian, menggunakan fallback...",
        err,
      );
    }
    ...
    if (modeToken === "siswa") {
      ...
      if (!realSiswaToken && techId) {
        try {
          ...
          const foundKey = Object.keys(savedMap).find(
            (k) => k.toLowerCase().trim() === techId.toLowerCase().trim(),
          );
          if (foundKey) {
      ...
      const validIndividuToken = realSiswaToken
    ...
    if (isTokenValid) {
      ...
      if (modeToken === "siswa") {
        setTokenError(
          `Token Unik Siswa tidak valid! Pastikan Anda memasukkan Token Rahasia milik Anda sendiri yang benar.`,
        );
      } else {
        setTokenError(
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="p-6 md:p-8 bg-[#0d1527]/50 backdrop-blur-md rounded-2xl border border-slate-800/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 shadow-xl">
            <div className="flex items-center gap-5">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="primary"
                    className="text-[9px] font-display font-bold px-2 py-0.5 rounded-md uppercase"
                  >
                    PESERTA
                  </Badge>
                </div>
                <p className="text-xs text-slate-400 font-sans flex items-center gap-1.5">
                  <CreditCard className="w-3.5 h-3.5 text-cyan-400" /> TechID:{" "}
                  <span className="text-slate-200 font-display font-bold tracking-wider">
                    {techId}
                  </span>
                </p>
              </div>
            ...
            <div className="text-left sm:text-right border-t sm:border-t-0 border-slate-800/60 pt-4 sm:pt-0 w-full sm:w-auto">
              <p className="text-[10px] text-slate-500 font-display font-bold uppercase tracking-widest mb-1">
                MATA UJIAN TERPILIH
              </p>
              <span className="text-cyan-400 font-display font-bold text-sm uppercase">
            ...
            <div className="p-6 md:p-8 bg-[#0d1527]/60 backdrop-blur-md rounded-2xl border border-emerald-500/40 space-y-6 shadow-2xl font-sans">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {(completedExamInfo?.totalSoal ?? 0) > 0 && (
                    <div className="bg-cyan-400/10 text-cyan-400 border border-cyan-400/30 text-xs px-3 py-1 rounded-lg font-display font-bold tracking-wide flex items-center gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5" /> Berhasil menjawab{" "}
                      {completedExamInfo?.totalTerjawab ?? 0} dari{" "}
                      {completedExamInfo?.totalSoal ?? 0} soal
                    </div>
                  )}
                  ...
                  <div className="bg-slate-800/80 text-cyan-400 border border-cyan-400/30 text-xs px-3 py-1 rounded-lg font-display font-bold tracking-wide flex items-center gap-1.5">
                    ...
                    {completedExamInfo?.lamaPengerjaan}
                  </div>
                  ...
                  {completedExamInfo?.isLulus ? (
                    <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/40 text-xs px-3 py-1 font-display font-bold flex items-center gap-1.5">
                      <CheckCircle2 className="w-4 h-4" /> LULUS (KKM:{" "}
                      {completedExamInfo?.kkm})
                    </Badge>
                  ) : (
                    <Badge className="bg-rose-500/20 text-rose-400 border-rose-500/40 text-xs px-3 py-1 font-display font-bold flex items-center gap-1.5">
                      <XCircle className="w-4 h-4" /> TIDAK LULUS (KKM:{" "}
                      {completedExamInfo?.kkm})
                    </Badge>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-5 rounded-xl bg-[#030712]/80 border border-slate-800/80 space-y-2">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-display font-bold text-cyan-400">
                      {completedExamInfo?.nilaiPG ?? 0}
                    </span>
                    <span className="text-xs text-slate-500 font-sans">
                      / 100
                    </span>
                  </div>
                  {completedExamInfo?.jumlahBenar !== null &&
                    completedExamInfo?.jumlahBenar !== undefined && (
                      <p className="text-[11px] font-sans font-bold">
                        <span className="text-emerald-400">
                          {completedExamInfo.jumlahBenar} jawaban benar
                        </span>
                        <span className="text-slate-500"> • </span>
                        <span className="text-rose-400">
                          {completedExamInfo.jumlahSalah ?? 0} jawaban salah
                        </span>
                      </p>
                    )}
                </div>
                ...
                <div className="p-5 rounded-xl bg-[#030712]/80 border border-slate-800/80 space-y-2">
                  ...
                  <p className="text-sm font-display font-bold text-amber-400 pt-1">
                    ...
                    completedExamInfo?.nilaiPraktik !== undefined ? (
                      <span className="text-3xl font-display font-bold text-amber-400">
                        {completedExamInfo.nilaiPraktik}
                      </span>
                    ) : (
                      <span className="text-xs font-sans text-amber-400/90">
                        {completedExamInfo?.statusPraktikText ??
                          "Dalam Koreksi Pengawas"}
                      </span>
                    )}
                  </p>
                ...
                <div
                  className={`p-5 rounded-xl bg-[#030712]/80 border space-y-2 ${completedExamInfo?.isLulus ? "border-emerald-500/30" : "border-rose-500/30"}`}
                >
                  <p
                    className={`text-[11px] font-display font-bold uppercase tracking-wider ${completedExamInfo?.isLulus ? "text-emerald-400" : "text-rose-400"}`}
                  >
                    NILAI AKHIR TOTAL
                  </p>
                  <div className="flex items-baseline gap-2">
                    <span
                      className={`text-3xl font-display font-bold ${completedExamInfo?.isLulus ? "text-emerald-400" : "text-rose-400"}`}
                    >
                      {completedExamInfo?.totalNilai ?? 0}
                    </span>
                    <span className="text-xs text-slate-500 font-sans">
                      (Batas KKM: {completedExamInfo?.kkm})
                    </span>
                  </div>
                </div>
              </div>
              ...
              {(completedExamInfo?.jumlahPindahTab ?? 0) > 0 && (
                <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-[11px] text-amber-400 font-sans flex items-center gap-2">
                  ...
                  Tercatat berpindah tab/aplikasi lain sebanyak{" "}
                  <strong>{completedExamInfo.jumlahPindahTab}x</strong> selama
                  pengerjaan.
                </div>
              )}
              ...
              {completedExamInfo?.isLulus && (
                <div className="flex justify-end">
                  <Button
                    ...
                    className="bg-emerald-400 hover:bg-emerald-300 text-slate-950 font-display font-bold text-xs border-0 flex items-center gap-2"
                  >
              ...
              <div className="space-y-3 font-sans">
                <div className="flex items-center gap-2 text-cyan-400 px-1">
                  ...
                  <h3 className="text-xs font-display font-bold uppercase tracking-widest">
                    MATA UJIAN ANDA (SESUAI DATA PENGAWAS)
                  </h3>
                </div>
