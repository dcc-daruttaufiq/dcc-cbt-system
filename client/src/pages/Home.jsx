...
// 2. Dummy Sample Showcase Karya Santri (Hall of Fame)
const karyaSantri = [
  {
    id: 1,
    ...
    gambar: 'https://images.unsplash.com/photo-1626785774573-4b799315345d?auto=format&fit=crop&w=600&q=80'
  },
  {
    id: 2,
    ...
    gambar: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=600&q=80'
  },
  {
    id: 3,
    ...
    gambar: 'https://images.unsplash.com/photo-1507238691740-187a5b1d37b8?auto=format&fit=crop&w=600&q=80'
  }
];
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-100 font-['Poppins',sans-serif] pb-20">
      ...
      {/* 📢 BANNER BROADCAST PENGUMUMAN TERKINI */}
      <div className="bg-gradient-to-r from-cyan-950/80 via-slate-900 to-cyan-950/80 border-b border-cyan-500/20 py-2.5 px-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between text-xs">
          <div className="flex items-center gap-2 overflow-hidden text-slate-300">
            <span className="flex items-center gap-1 font-['Rajdhani',sans-serif] font-bold text-cyan-400 bg-cyan-400/10 px-2 py-0.5 rounded border border-cyan-400/30 uppercase shrink-0">
              <Megaphone className="w-3.5 h-3.5" /> BROADCAST
            </span>
            <p className="truncate font-['Poppins',sans-serif] text-[11px] md:text-xs">
              Sesi Ujian Sertifikasi Gelombang II Berjalan Lancar | Pastikan Scan Presensi Sebelum Masuk Lab!
            </p>
          </div>
          <span className="text-[10px] text-slate-500 hidden md:block shrink-0 font-['Rajdhani',sans-serif]">DCC SYSTEM V2.4</span>
        </div>
      </div>

      {/* 🚀 HERO SECTION */}
      <div className="flex flex-col items-center justify-center text-center px-6 pt-12 pb-10">
        <motion.div
        ...
        >
          ...
          <p className="mt-4 text-xs md:text-sm text-slate-400 max-w-xl leading-relaxed">
            Mewujudkan santri yang berakhlak, adaptif, dan unggul di bidang komputer. Akses seluruh portal layanan terpadu dalam satu sistem terintegrasi.
          </p>
        </motion.div>

        {/* 📊 BARIS STATISTIK RINGKAS (LIVE COUNTER) */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex flex-col items-center p-3 border-r border-slate-800/80 last:border-r-0">
            ...
            <span className="font-['Rajdhani',sans-serif] text-2xl font-bold text-white">150+</span>
            <span className="text-[10px] text-slate-400 uppercase tracking-wider font-['Rajdhani',sans-serif]">Santri Aktif</span>
          </div>
          <div className="flex flex-col items-center p-3 border-r border-slate-800/80 last:border-r-0">
            <CheckCircle2 className="w-5 h-5 text-emerald-400 mb-1" />
            <span className="font-['Rajdhani',sans-serif] text-2xl font-bold text-emerald-400">98%</span>
            <span className="text-[10px] text-slate-400 uppercase tracking-wider font-['Rajdhani',sans-serif]">Kelulusan CBT</span>
          </div>
          <div className="flex flex-col items-center p-3 border-r border-slate-800/80 last:border-r-0">
            <MonitorCheck className="w-5 h-5 text-amber-400 mb-1" />
            <span className="font-['Rajdhani',sans-serif] text-2xl font-bold text-amber-400">40 Unit</span>
            <span className="text-[10px] text-slate-400 uppercase tracking-wider font-['Rajdhani',sans-serif]">Komputer Ready</span>
          </div>
          <div className="flex flex-col items-center p-3">
            <ShieldCheck className="w-5 h-5 text-indigo-400 mb-1" />
            <span className="font-['Rajdhani',sans-serif] text-2xl font-bold text-indigo-400">Official</span>
            <span className="text-[10px] text-slate-400 uppercase tracking-wider font-['Rajdhani',sans-serif]">Sertifikasi Lab</span>
          </div>
        </motion.div>
      </div>
        ...
        <div className="flex items-center justify-between mb-6 border-b border-slate-800/80 pb-3">
          <div>
            <h2 className="font-['Rajdhani',sans-serif] text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-cyan-400" /> Modul & Portal Utama
            </h2>
            <p className="text-xs text-slate-400">Pilih sistem yang ingin diakses sesuai dengan wewenang Anda.</p>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 mt-16">
        <div className="flex items-center justify-between mb-6 border-b border-slate-800/80 pb-3">
          <div>
            <h2 className="font-['Rajdhani',sans-serif] text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Award className="w-5 h-5 text-amber-400" /> DCC Hall of Fame (Karya Santri)
            </h2>
            <p className="text-xs text-slate-400">Apresiasi hasil karya praktik terbaik santri Daruttaufiq Computer Centre.</p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {karyaSantri.map((item) => (
            <div key={item.id} className="bg-[#0d1527]/80 border border-slate-800 rounded-2xl overflow-hidden shadow-xl hover:border-slate-700 transition group">
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="flex items-center justify-between text-xs text-slate-400 border-t border-slate-800/80 pt-2.5 font-['Poppins',sans-serif]">
                  ...
                  <span className="font-['Rajdhani',sans-serif] font-bold text-slate-500">{item.techId}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <footer className="mt-20 border-t border-slate-800/80 pt-8 text-center text-xs text-slate-500 font-['Poppins',sans-serif]">
        <p>© {new Date().getFullYear()} Daruttaufiq Computer Centre. All rights reserved.</p>
        <p className="text-[11px] text-slate-600 mt-1">Sistem Terintegrasi Laboratorium Komputer & Pendidikan Santri</p>
      </footer>
    </div>
  );
}