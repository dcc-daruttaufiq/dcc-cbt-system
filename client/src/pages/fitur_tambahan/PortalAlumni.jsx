      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-12">
          ...
          {loading ? (
            ...
            <p className="text-xs text-slate-500">Belum ada data alumni. Insert manual 1 baris di tabel `alumni` untuk uji coba.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {alumni.map((a, idx) => (
                ...
                >
                  <span className={`w-fit text-[9px] font-['Rajdhani',sans-serif] font-bold px-2 py-0.5 rounded-md border uppercase tracking-wider ${kategoriWarna[a.kategori] || kategoriWarna.Studi}`}>
                    {a.kategori}
                  </span>
                  <div>
        ...
        <div>
          ...
          {loading ? (
            ...
            <p className="text-xs text-slate-500">Belum ada lowongan magang aktif. Insert manual di tabel `lowongan_magang` untuk uji coba.</p>
          ) : (
            <div className="space-y-3">
              {lowongan.map((l) => (
                <div key={l.id} className="bg-[#0d1527]/70 border border-slate-800 rounded-xl p-4 flex items-center justify-between gap-4">
                  <div>
