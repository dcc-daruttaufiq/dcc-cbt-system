...
import {
  MonitorCheck, Plus, X, Search, Trash2, Save,
  Home, CheckSquare, Database, Sliders, FileBarChart, AlertTriangle, Wrench
} from 'lucide-react';

const FASILITAS_TABLE = 'fasilitas_dcc';
const AKUN_SESSION_KEY = 'dcc_akun_session';

const KATEGORI_OPSI = ['Komputer', 'Jaringan', 'Elektronik', 'Ruangan', 'Furnitur', 'Lainnya'];
...
export default function FasilitasDCC() {
  ...
  const handleHapus = async (id) => {
    ...
    try {
      const { error } = await supabase.from(FASILITAS_TABLE).delete().eq('id', id);
      if (error) throw error;
  ...
  const daftarTerfilter = daftarFasilitas.filter((f) => {
    ...
    if (searchQuery.trim()) {
      ...
      return f.nama_fasilitas.toLowerCase().includes(q) || (f.catatan || '').toLowerCase().includes(q);
    }
    return true;
  });

  const getStatusBadge = (s) => {
    if (s === 'Rusak') return { className: 'bg-rose-500/20 text-rose-400 border-rose-500/40', icon: AlertTriangle };
    if (s === 'Perlu Perbaikan') return { className: 'bg-amber-500/20 text-amber-400 border-amber-500/40', icon: Wrench };
    return { className: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/40', icon: CheckSquare };
  };
        ...
        <Navbar>
          <div className="flex justify-between items-center w-full">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-display font-bold text-white tracking-wide">FASILITAS & ASET DCC</h1>
                <p className="text-xs text-slate-400">Catatan kondisi perangkat & fasilitas laboratorium</p>
              </div>
            </div>
            <Button onClick={openTambahModal} className="text-xs bg-cyan-400 hover:bg-cyan-300 text-slate-950 font-display font-bold border-0 flex items-center gap-1.5">
        ...
        <main className="p-6 md:p-8 flex-1 overflow-y-auto">
          <div className="max-w-5xl mx-auto space-y-5">
            ...
            <div className="space-y-3">
              {isLoading ? (
                <div className="p-10 text-center text-xs text-slate-500">Memuat data fasilitas...</div>
              ) : daftarTerfilter.length === 0 ? (
                <div className="p-10 text-center text-xs text-slate-500 bg-[#0d1527]/40 rounded-2xl border border-slate-800">Belum ada catatan fasilitas pada filter ini.</div>
              ) : (
                daftarTerfilter.map((f) => {
                  const badge = getStatusBadge(f.status);
                  const BadgeIcon = badge.icon;
                  return (
                    <div key={f.id} className="p-4 bg-[#0d1527]/60 border border-slate-800/60 rounded-2xl flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div className="flex-1 min-w-0 space-y-1.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="text-sm font-display font-bold text-white">{f.nama_fasilitas}</h3>
                          <Badge className="bg-slate-800 text-slate-300 text-[9px] px-2 py-0.5 rounded-md border border-slate-700">{f.kategori}</Badge>
                          <Badge className={`text-[9px] px-2 py-0.5 rounded-md border flex items-center gap-1 ${badge.className}`}>
                            <BadgeIcon className="w-2.5 h-2.5" /> {f.status}
                          </Badge>
                        </div>
                        {f.catatan && <p className="text-xs text-slate-400 leading-relaxed">{f.catatan}</p>}
                        <p className="text-[10px] text-slate-500">Dicatat oleh {f.dicatat_oleh} • {formatTanggal(f.created_at)}</p>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-[#0d1527] border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800/60">
              <h3 className="font-display text-base font-bold text-cyan-400 uppercase">Catat Fasilitas</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>

            <form onSubmit={handleSimpan} className="space-y-4">
              <Input label="Nama Fasilitas" placeholder="Contoh: Komputer Unit 05" value={namaFasilitas} onChange={(e) => setNamaFasilitas(e.target.value)} required />

              <div>
                <label className="text-xs font-display font-bold text-slate-300 mb-1.5 block uppercase">Kategori</label>
                  ...
                  {KATEGORI_OPSI.map((k) => (
                    <button
                      ...
                      className={`py-2 rounded-xl border text-[11px] font-display font-bold transition ${
                        kategori === k ? 'bg-cyan-400 text-slate-950 border-cyan-400' : 'text-slate-400 border-slate-800 hover:text-white'
                      }`}
                    >
                      {k}
                    </button>
              ...
              <div>
                <label className="text-xs font-display font-bold text-slate-300 mb-1.5 block uppercase">Status Kondisi</label>
                <div className="grid grid-cols-3 gap-2">
                  {['Baik', 'Perlu Perbaikan', 'Rusak'].map((s) => (
                    <button
                      ...
                      className={`py-2 rounded-xl border text-[11px] font-display font-bold transition ${
                        status === s
                          ? s === 'Rusak' ? 'bg-rose-500 text-white border-rose-500'
                          : s === 'Perlu Perbaikan' ? 'bg-amber-400 text-slate-950 border-amber-400'
              ...
              <div>
                <label className="text-xs font-display font-bold text-slate-300 mb-1.5 block uppercase">Catatan (opsional)</label>
                <textarea
                  rows={3}
                  value={catatan}
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800/60">
                ...
                <Button type="submit" disabled={isSubmitting} className="bg-cyan-400 hover:bg-cyan-300 text-slate-950 font-display font-bold text-xs border-0 flex items-center gap-1.5">
