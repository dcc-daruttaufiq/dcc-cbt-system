...
export default function NilaiAkhir() {
  ...
  const getKkmMataUjian = (katId, katalogList) => {
    ...
    return matchedKat?.kkm !== undefined && matchedKat?.kkm !== null
      ? Number(matchedKat.kkm)
      : 75;
  };
  ...
  const calculateNilaiAkhirSmart = (pesertaItem, katalogList) => {
    ...
    const nilaiPG =
      pesertaItem.nilai_pg !== undefined && pesertaItem.nilai_pg !== null
        ? Number(pesertaItem.nilai_pg)
        : 0;
    const nilaiPraktik =
      pesertaItem.nilai_praktik !== undefined &&
      pesertaItem.nilai_praktik !== null
        ? Number(pesertaItem.nilai_praktik)
        : null;

    // Jika nilai_akhir sudah tersimpan dari proses koreksi, prioritaskan nilai tersebut
    if (
      pesertaItem.nilai_akhir !== undefined &&
      pesertaItem.nilai_akhir !== null &&
      Number(pesertaItem.nilai_akhir) > 0
    ) {
    ...
    return Math.round(
      nilaiPG * (bobotPG / 100) + nilaiPraktik * (bobotPraktik / 100),
    );
  };
  ...
  const fetchLaporan = async () => {
    ...
    try {
      ...
      if (dataKat && dataKat.value) {
        const parsed =
          typeof dataKat.value === "string"
            ? JSON.parse(dataKat.value)
            : dataKat.value;
    ...
    try {
      const { data, error } = await supabase.from(TABLES.PESERTA).select("*");

      if (!error && data && data.length > 0) {
        dataReal = data.filter(
          (p) =>
            p.status === "selesai" ||
            Number(p.nilai_akhir) > 0 ||
            Number(p.nilai_pg) > 0,
        );
      }
    } catch (err) {
    ...
    if (dataReal.length === 0) {
      const localSesi =
        ...
        localStorage.getItem("dcc_sesi_peserta");
      if (localSesi) {
        try {
          const parsed = JSON.parse(localSesi);
          if (Array.isArray(parsed)) {
            dataReal = parsed.filter(
              (p) =>
                p.status === "selesai" ||
                Number(p.nilai_akhir) > 0 ||
                Number(p.nilai_pg) > 0,
    ...
    const totalNilai = dataReal.reduce(
      (acc, curr) => acc + calculateNilaiAkhirSmart(curr, currentKatalog),
      0,
    );
    const rataRata = totalSiswa > 0 ? Math.round(totalNilai / totalSiswa) : 0;
    const nilaiList = dataReal.map((d) =>
      calculateNilaiAkhirSmart(d, currentKatalog),
    );
  ...
  const handleExportExcel = () => {
    if (laporan.dataLaporan.length === 0)
      return alert("Belum ada data nilai peserta untuk diexport!");
    ...
    const dataExcel = laporan.dataLaporan.map((item, index) => {
      ...
      return {
        ...
        "Nama Lengkap":
          item.nama || item.nama_lengkap || `Peserta #${index + 1}`,
        "Mata Ujian": getLabelKategori(item.kategori),
        KKM: kkmReal,
        "Nilai PG": item.nilai_pg || 0,
        "Nilai Praktik":
          item.nilai_praktik !== null && item.nilai_praktik !== undefined
            ? item.nilai_praktik
            : "Belum Dikoreksi",
  ...
  const handleExportPDF = () => {
    ...
    if (!printWindow) {
      alert(
        "Pop-up diblokir oleh browser. Izinkan pop-up untuk mengunduh PDF.",
      );
      return;
    }
          ...
          <div className="flex justify-between items-center w-full">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-sm font-display font-bold text-white tracking-wide">
                  LAPORAN & RANKING UJIAN
                </h1>
                <p className="text-[11px] text-slate-400 font-sans">
                  Analitik Realtime Hasil Ujian Peserta
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className="bg-slate-800 hover:bg-slate-700 text-xs text-slate-300 border-0 font-sans"
              >
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-4 bg-[#0d1527]/60 border border-slate-800/60 rounded-2xl flex items-center gap-4">
                ...
                <div>
                  <p className="text-[11px] text-slate-400 uppercase font-semibold font-sans">
                    Total Peserta Selesai
                  </p>
                  <h3 className="text-lg font-display font-bold text-white">
                    {laporan.statistik.totalSiswa} Siswa
                  </h3>
                </div>
              </div>
              ...
              <div className="p-4 bg-[#0d1527]/60 border border-slate-800/60 rounded-2xl flex items-center gap-4">
                ...
                <div>
                  ...
                  <h3 className="text-lg font-display font-bold text-emerald-400">
                    {laporan.statistik.tertinggi}
                  </h3>
                </div>
              </div>
              ...
              <div className="p-4 bg-[#0d1527]/60 border border-slate-800/60 rounded-2xl flex items-center gap-4">
                ...
                <div>
                  ...
                  <h3 className="text-lg font-display font-bold text-rose-400">
                    {laporan.statistik.terendah}
                  </h3>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {laporan.dataLaporan.length === 0 ? (
                ...
                laporan.dataLaporan.map((row, idx) => {
                  const nilaiAkhir = calculateNilaiAkhirSmart(
                    row,
                    katalogMapel,
                  );
                  ...
                  const namaSiswa =
                    row.nama || row.nama_lengkap || `Peserta #${idx + 1}`;
                  const techId = row.tech_id || `-`;

                  return (
                    <div
                      key={idx}
                      className="p-5 bg-[#0d1527]/60 border border-slate-800/50 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 font-sans"
                    >
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="space-y-1 min-w-[200px]">
                          ...
                          <p className="text-[11px] text-slate-400 flex items-center gap-1 font-sans">
                            <CreditCard className="w-3 h-3 text-cyan-400 inline" />{" "}
                            TechID:{" "}
                            <span className="text-slate-200 font-semibold font-display">
                              {techId}
                            </span>
                          </p>
                        </div>
                        ...
                        <div className="w-32 shrink-0 hidden sm:block">
                          <Badge
                            className={`text-[10px] font-display font-bold px-2.5 py-1 rounded-md uppercase text-center w-full block border ${
                              isLulus
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-6 shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-slate-800/40">
                        <div className="flex items-center gap-6">
                          <div className="text-right">
                            <p className="text-[10px] text-amber-400 font-display uppercase font-bold">
                              KKM
                            </p>
                          ...
                          <div className="text-right">
                            <p className="text-[10px] text-slate-400 font-display uppercase">
                              Nilai PG
                            </p>
                          ...
                          <div className="text-right pl-4 border-l border-slate-800/60">
                            <p className="text-[10px] text-cyan-400 font-display uppercase font-bold">
                              Nilai Akhir
                            </p>
